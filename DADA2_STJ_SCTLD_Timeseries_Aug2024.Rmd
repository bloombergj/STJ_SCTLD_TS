...
sbatch --cpus-per-task=4 --mem=32gb --time=03:10:00 /proj/omics/bioinfo/scripts/slurm/singularity_launch_rstudio.sh
...
#TOTAL READS using "zcat *fastq.gz | echo $((`wc -l`/4))":
```
...
length(list.files(path))
  #436 -- perfect bc there are 218 samples, and 218*2=436
...
sample.names1 <- sapply(strsplit(basename(fnFs[1:30]), "_"), function(x) ifelse(length(x) > 1, paste(x[1], x[2], sep = "_"), x))
sample.names2 <- sapply(strsplit(basename(fnFs[31:32]), "_"), function(x) ifelse(length(x) > 1, paste(x[1], x[2], x[3], sep = "_"), x))
sample.names3 <- sapply(strsplit(basename(fnFs[33:42]), "_"), function(x) ifelse(length(x) > 1, paste(x[1], x[2], sep = "_"), x))
sample.names4 <- sapply(strsplit(basename(fnFs[43:95]), "_"), `[`, 1)
sample.names5 <- sapply(strsplit(basename(fnFs[96:101]), "_"), function(x) ifelse(length(x) > 1, paste(x[1], x[2], sep = "_"), x))
sample.names6 <- sapply(strsplit(basename(fnFs[102:103]), "_"), `[`, 1)
sample.names7 <- sapply(strsplit(basename(fnFs[104]), "-"), `[`, 1)
sample.names8 <- sapply(strsplit(basename(fnFs[105:168]), "_"), `[`, 1)
sample.names9 <-  sapply(strsplit(basename(fnFs[169:215]), "_"), function(x) ifelse(length(x) > 1, paste(x[1], x[2], sep = "_"), x))
sample.names10 <-  sapply(strsplit(basename(fnFs[216:218]), "_"), function(x) ifelse(length(x) > 1, paste(x[1], x[2], x[3], sep = "_"), x))
...
errF_1run <- learnErrors(filtFs_1run, multithread=3)
      #121435440 total bases in 505981 reads from 3 samples will be used for learning the error rates.
errF_2run <- learnErrors(filtFs_2run, multithread=3)
      #105232560 total bases in 438469 reads from 2 samples will be used for learning the error rates.
errF_3run <- learnErrors(filtFs_3run, multithread=3)
      #103181040 total bases in 429921 reads from 24 samples will be used for learning the error rates.
errF_4run <- learnErrors(filtFs_4run, multithread=3)
      #106168560 total bases in 442369 reads from 12samples will be used for learning the error rates.
errF_5run <- learnErrors(filtFs_5run, multithread=3)
      #105691200 total bases in 440380 reads from 14 samples will be used for learning the error rates.
errF_6run <- learnErrors(filtFs_6run, multithread=3)
...
```{r}
dadaFs_1run <- dada(filtFs_1run, err=errF_1run, multithread=3)
dadaRs_1run <- dada(filtRs_1run, err=errR_1run, multithread=3)
dadaFs_1run[[12]]

dadaFs_2run <- dada(filtFs_2run, err=errF_2run, multithread=3)
dadaRs_2run <- dada(filtRs_2run, err=errR_2run, multithread=3)
dadaFs_2run[[12]]

dadaFs_3run <- dada(filtFs_3run, err=errF_3run, multithread=3)
dadaRs_3run <- dada(filtRs_3run, err=errR_3run, multithread=3)
dadaFs_3run[[12]]

dadaFs_4run <- dada(filtFs_4run, err=errF_4run, multithread=3)
dadaRs_4run <- dada(filtRs_4run, err=errR_4run, multithread=3)
dadaFs_4run[[12]]

dadaFs_5run <- dada(filtFs_5run, err=errF_5run, multithread=3)
dadaRs_5run <- dada(filtRs_5run, err=errR_5run, multithread=3)
dadaFs_5run[[12]]

dadaFs_6run <- dada(filtFs_6run, err=errF_6run, multithread=3)
dadaRs_6run <- dada(filtRs_6run, err=errR_6run, multithread=3)
dadaFs_6run[[12]]
...
#  Identified 472 bimeras out of 24356 input sequences.
...
```{r}
taxa <- assignTaxonomy(seqtab_final, "/Users/cmiller/Downloads/temp_jeanne/silva_nr99_v138.1_train_set.fa.gz", multithread=TRUE)
    #this was >10 min on Carolyn's computer!!!! 
taxa <- addSpecies(taxa, "/Users/cmiller/Downloads/temp_jeanne/silva_species_assignment_v138.1.fa.gz")
    #this was >10 min on Carolyn's computer!!!! 

#save(taxa, file="/Users/cmiller/Downloads/temp_jeanne/taxaAssignTax.RData")
