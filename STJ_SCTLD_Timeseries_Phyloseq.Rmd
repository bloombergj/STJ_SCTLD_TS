...
mod <- lm(Axis1 ~ Days_Since_First + Tag + Tag*Days_Since_First, data = PCoA_df) 
summary(mod)
      #                             Estimate Std. Error t value Pr(>|t|)   
      # (Intercept)              -0.6540086  0.2727086  -2.398  0.02365 * 
      # Days_Since_First          0.0008921  0.0004652   1.918  0.06577 .

      # Multiple R-squared:  0.371,	Adjusted R-squared:  0.1147 
      # F-statistic: 1.448 on 11 and 27 DF,  p-value: 0.2091
```
...
mod <- lm(Axis2 ~ Days_Since_First + Tag +  Tag*Days_Since_First, data = PCoA_df) 
summary(mod)
      #                          Estimate Std. Error t value Pr(>|t|)  
      # (Intercept)               -1.6511     0.7305  -2.260   0.0303 *
      # Days_Since_First           0.2241     0.1037   2.162   0.0378 *

      # Multiple R-squared:  0.275,	Adjusted R-squared:  0.0405 
      # F-statistic: 1.173 on 11 and 34 DF,  p-value: 0.3412
```
...
Graphical inspection. Corak Bay, seawater x Date. Chose Axis 4 or 5
```{r}
PCoA_df$Days_Since_First <- as.factor(PCoA_df$Days_Since_First) #only do this for visualization purposes. Need to change it back to num before LM.

#I did a lot of interations, so I wrote down results --> 2v5 is indicative of why I chose Axis 5
ggplot(data = PCoA_df, aes(x=Axis2, y=Axis5, color=Days_Since_First)) +
  geom_point(size = 4) +  
  theme_bw() +
  theme(text=element_text(size=17),  
        legend.text=element_text(size=17), 
...
LM with Axis 4 response (Coral Bay, seawater, testing effect of Date)
```{r}
PCoA_df$Tag <- as.factor(PCoA_df$Tag)
str(PCoA_df$Tag)
PCoA_df$Days_Since_First <- as.numeric(PCoA_df$Days_Since_First)
...
mod <- lm(Axis5 ~ Days_Since_First + Tag, data = PCoA_df) 
summary(mod)
      #                   Estimate Std. Error t value Pr(>|t|)  
      # (Intercept)      -0.067528   0.033380  -2.023   0.0502 .
      # Days_Since_First  0.008677   0.003892   2.229   0.0318 *

      # Residual standard error: 0.07279 on 38 degrees of freedom
      # Multiple R-squared:  0.1483,	Adjusted R-squared:  0.01379 
      # F-statistic: 1.103 on 6 and 38 DF,  p-value: 0.3789
```
Date has an effect on the near-coral seawater at Coral Bay (β = 0.008677, SE = 0.003892, t = 2.229, p = 0.0318)
...
summary(mod)
    #                            Estimate Std. Error t value Pr(>|t|)  
    # (Intercept)               2.003e-01  1.586e-01   1.263   0.2287  
    # Days_Since_First         -1.223e-03  6.328e-04  -1.932   0.0754 

    # Multiple R-squared:  0.338,	Adjusted R-squared:  -0.2222 
    # F-statistic: 0.6034 on 11 and 13 DF,  p-value: 0.796
```
