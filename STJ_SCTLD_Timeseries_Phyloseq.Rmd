...
rm(list = setdiff(ls(), c("ps.decontam.filt", "ps.coral.Fishbay", "taxa")))
```{r}
...
mean_per_sample_FB <- df_ps_top_ASVs %>% 
                  group_by(Sample) %>%
...
rm(list = setdiff(ls(), c("ps.decontam.filt", "ps.seawater.Fishbay", "taxa")))
```{r}
