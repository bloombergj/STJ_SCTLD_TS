...
This code is for a special case. In the field, one syringe of seawater was filtered through two different filters, so they the two sequences are actually representative of one sample. I will combine the two samples into one. 
```{r}
...
-------------------this is as far as I got on 8/21/24
...
I am choosing the sample with more reads, but I think it only matters when there are orders
of magnitudes in difference. 
...
duplicates_toRemove <- c("Coral_52_1_ACTCACTG", "SW-26", "SW-35", "SW-8duplicate")
...
save(otus_noDuplicates, file="~/Documents/WHOI/Projects/STJ_SCTLD_TimeseriesExperiment/Analysis_Code/otus_STJSCTLD_final.RData")
...
taxa <- as.matrix(read.table("taxonomy_STJSCTLD_final.txt", sep = "\t", row.names = 1, header = TRUE))

ASV_ID <-read.table("ASVsequences_STJSCTLD_final.txt", sep = "\t", row.names = 1, header = TRUE)

metadata <- read.csv("meta_table.csv", header = TRUE)
metadata <- metadata[1:164, 1:24]
row.names(metadata) <- metadata$Sample_names
metadata <- metadata[samples_we_use, ]

#metadata$Date_nice <- as.Date(metadata$Date , format = "%Y/%m/%d")
...
#remove contaminants
load("~/Documents/WHOI/Projects/STJ_SCTLD_TimeseriesExperiment/Analysis_Code/contam.RData")
...
ps.decontam.nozero <- filter_taxa(ps.decontam, function(x) sum(x) > 0, TRUE)
  #taxa: 20125 taxa
  #samples: 139

    paste("Number of ASVs in the dataset (without contaminants) prior to removing ASVs not present in any sample =", ntaxa(ps.decontam))
      #20430
    paste("Number of ASVs in the dataset (without contaminants) after removing ASVs not present in any sample =", ntaxa(ps.decontam.nozero))
      #20125
    paste("the number of ASVs not present in any sample that were removed =", ntaxa(ps.decontam)-ntaxa(ps.decontam.nozero))
      #305
    
#Remove the samples I don't want to use: (14 samples removed)
    #Coral_48 --> only diseased sample at Coral Bay
    #Seawater_61 --> only diseased sample at Coral Bay
    #Seawater_68 --> only dead sample at Coral Bay
    #Tag 1534 (4 samples)
    #Tag 2300 (2 samples)
    #HealthState DH (4) --> only going to remove these for coral samples, not SW
 
 ps.decontam.filt <- ps.decontam.nozero %>%
    subset_samples(Sample_ID !="Coral_48" & 
                   Sample_ID !="Seawater_61" &
                   Sample_ID !="Seawater_68" & 
                   Tag !="1534" & 
                   Tag !="2300")
  ...
  ps.nocontrol.nomirochloro <- ps.decontam.nozero %>%
    subset_samples(Sample_ID !="Coral_48" & 
                   Sample_ID !="Seawater_61" &
                   Sample_ID !="Seawater_68" & 
                   Tag !="1534" & 
                   ...
                   subset_samples(Sample_Type != "Coral")
 
 Reads_per_sample <- as.data.frame(sample_sums(ps.nocontrol.nomirochloro))
 mean(Reads_per_sample$`sample_sums(ps.nocontrol.nomirochloro)`)  #85306.06
 sd(Reads_per_sample$`sample_sums(ps.nocontrol.nomirochloro)`)  #46542.01
 sum(Reads_per_sample$`sample_sums(ps.nocontrol.nomirochloro)`)
```

Sums_all <- as.data.frame(rowSums(otu_table(ps.decontam.nozero)))
...
perm.C.FB.HDR 
    ...
    #      Number of permutations: 999
    #                                Df SumOfSqs     R2      F Pr(>F)  
    #      Healthy_Diseased_Recovered  2   1.1282 0.0911 1.2027  0.019 *
    #      Residual                   24  11.2563 0.9089                
    #      Total                      26  12.3845 1.0000  
...
perm.C.FB.date 
    #   Number of permutations: 999
    #             Df SumOfSqs      R2      F Pr(>F)  
    #    Date      6   3.0436 0.24576 1.0861  0.048 *
    #    Residual 20   9.3409 0.75424                
    #    Total    26  12.3845 1.00000                
...
perm.SW.FB.HDR 
    ...
    #    Number of permutations: 999
    #                                  Df SumOfSqs      R2      F Pr(>F)  
    #      Healthy_Diseased_Recovered  3   1.3104 0.26819 3.2982  0.016 *
    #      Residual                   27   3.5758 0.73181                
    #      Total                      30   4.8862 1.00000  
...
perm.SW.FB.date 
    ...
    #    Number of permutations: 999
    #             Df SumOfSqs      R2     F Pr(>F)   
    #    Date      6   2.0039 0.41012 2.781  0.002 **
    #    Residual 24   2.8823 0.58988                
    #    Total    30   4.8862 1.0000
...
perm.SW.CB.HDR 
    ...
    #       Number of permutations: 999
    #                         Df SumOfSqs      R2      F Pr(>F)   
    #        Site_Disease  1  0.38847 0.17202 6.8558  0.001 ***
    #        Residual     33  1.86989 0.82798                  
    #        Total        34  2.25837 1.00000  
...
perm.SW.CB.date 
  ...
  #      Number of permutations: 999
  #               Df SumOfSqs      R2     F Pr(>F)    
  #      Date      7  1.92057 0.85043 21.93  0.001 ***
  #      Residual 27  0.33779 0.14957                 
  #      Total    34  2.25837 1.00000   
...
perm.C.CB.HDR 
  ...
  #        Number of permutations: 999
  #                         Df SumOfSqs      R2      F Pr(>F)
  #          Site_Disease  1    0.540 0.03907 1.1384   0.14
  #          Residual     28   13.282 0.96093              
  #          Total        29   13.822 1.00000 
...
perm.C.CB.date 
      ...
      #    Number of permutations: 999
      #             Df SumOfSqs      R2      F Pr(>F)  
      #    Date      6   3.2359 0.23411 1.1717  0.015 *
      #    Residual 23  10.5861 0.76589                
      #    Total    29  13.8220 1.00000  
