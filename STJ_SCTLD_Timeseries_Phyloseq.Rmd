...
#Remove the samples I don't want to use: (13 samples removed)
 ...
 mean(Reads_per_sample$`sample_sums(ps.decontam.filt)`)  #65397.13
 sd(Reads_per_sample$`sample_sums(ps.decontam.filt)`)  #64605.68
 sum(Reads_per_sample$`sample_sums(ps.decontam.filt)`) #11640690
...
perm.C.FB.HDR 
    ...
    #      Number of permutations: 999
    # Healthy_Diseased_Recovered  2    1.062 0.05602 1.1275  0.029 *
    # Residual                   38   17.895 0.94398                
    # Total                      40   18.957 1.00000      
...
perm.C.FB.date 
    #          Df SumOfSqs      R2      F Pr(>F)   
    # Date     10   5.1149 0.26982 1.1086  0.005 **
    # Residual 30  13.8422 0.73018                 
    # Total    40  18.9571 1.00000                    
...
perm.SW.FB.HDR 
    ...
    #    Number of permutations: 999
    # Healthy_Diseased_Recovered  3   0.9929 0.15519 2.5718  0.013 *
...
perm.SW.FB.date 
    ...
    #    Number of permutations: 999
    # Date     10   3.2742 0.51175 3.6685  0.001 ***
...
perm.C.CB.HDR 
  ...
  #        Number of permutations: 999
  #              Df SumOfSqs      R2      F Pr(>F)   
  # Site_Disease  1   0.6044 0.03696 1.2664  0.005 **
...
plot <- ggplot()+
  geom_point(data = dis2.meta, aes(x = Date_nice, y=distance), color="Blue") +
  geom_smooth(data = dis2.meta, aes(x = Date_nice, y=distance), method = "lm", color="Blue", se = FALSE) +
...
# Transpose the abundance table if necessary to have ASVs in rows and samples in columns
abundance_table <- t(abundance_table)

# Add a column for ASV IDs
abundance_table <- as.data.frame(abundance_table) #redundant, but keeps R happy
abundance_table$ASV <- rownames(abundance_table)
...
length(unique(abundance_long$ASV)) #132 --- ok much more reasonable 
n <- 132 #plug that number in here to a get that number of unique colors to plot
palette <- distinctColorPalette(n)
  
# Create the stacked bar plot
ggplot(abundance_long, aes(x = Sample, y = Abundance, fill = ASV)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  scale_fill_manual(values=palette) +
  labs(x = "Sample", y = "Relative Abundance", fill = "ASV") +
