...
```{r}
PCoA_df <- data.frame(SampleID = rownames(SWandCoral.BothSites.ord$vectors), 
                      Axis1 = SWandCoral.BothSites.ord$vectors[, 1], 
                      Axis2 = SWandCoral.BothSites.ord$vectors[, 2], 
                      Axis3 = SWandCoral.BothSites.ord$vectors[, 3], 
                      Site = sample_data(ps.SWandCoral.BothSites)$Site, 
                      Date = sample_data(ps.SWandCoral.BothSites)$Date, 
                      Tag = sample_data(ps.SWandCoral.BothSites)$Tag, 
                      Sample_Type = sample_data(ps.SWandCoral.BothSites)$Sample_Type)
...
graphical inspection --> chose axis 1
```{r}
...
LM with Axis 1 response (testing effect of sample type)
```{r}
...
mod <- lm(Axis1 ~ Sample_Type + Site + Days_Since_First + Tag , data = PCoA_df) 
summary(mod)
      # Coefficients: (1 not defined because of singularities)
      #                       Estimate Std. Error t value Pr(>|t|)    
      # (Intercept)         -3.504e-01  2.972e-02 -11.790   <2e-16 ***
      # Sample_TypeSeawater  6.151e-01  1.594e-02  38.584   <2e-16 ***
confint(mod)
      #                             2.5 %        97.5 %
      # (Intercept)         -4.091856e-01 -2.916685e-01
      # Sample_TypeSeawater  5.835531e-01  6.465820e-01
```
...
confint(mod)
      #                                  2.5 %        97.5 %
      # (Intercept)              -0.3432347126  4.114717e-02
      # SiteFish_Bay              0.0654742251  5.345788e-01
...
confint(mod)
    #                                  2.5 %        97.5 %
    # (Intercept)              -0.0216108983  0.1885760489
    # SiteFish_Bay             -0.3667929355 -0.0953652734
```
...
confint(mod)
      #                                  2.5 %        97.5 %
      # (Intercept)              -1.1091715693 -0.0465373434
      # HDRDiseased              -0.3050018390  0.0473116393
      # HDRRecovered             -0.4828710694 -0.0996781029
...
confint(mod)
      #                          2.5 %       97.5 %
      # (Intercept)      -0.3390833777 9.133350e-02
      # Days_Since_First -0.0002147632 6.531741e-05
```
...
confint(mod)
    #                                  2.5 %        97.5 %
    # (Intercept)              -0.1517836450  1.1132087723
    # HDRDead                  -0.2467377588  0.7628751770
    # HDRDiseased              -0.6006931744 -0.2385711774
    # HDRRecovered             -0.4688910798 -0.0309979804
...
confint(mod)
      #                          2.5 %       97.5 %
      # (Intercept)      -3.458248e-01 0.0096288683
      # Days_Since_First  4.823401e-05 0.0002638768
```
...
confint(mod)
      #                                  2.5 %        97.5 %
      # (Intercept)              -3.698470e-01 -0.0615823006
      # Site_DiseaseBefore        1.404382e-01  0.3288281677
```
...
confint(mod)
      #                        2.5 %       97.5 %
      # (Intercept)      -0.01994286  0.141762401
      # Days_Since_First -0.02466392 -0.005810665
```
...
confint(mod)
    #                                  2.5 %      97.5 %
    # (Intercept)              -0.6368397145 0.426105474
    # Site_DiseaseBefore       -0.0978942341 0.625459458
```
...
confint(mod)
    #                          2.5 %       97.5 %
    # (Intercept)      -0.2422304457 0.2062359205
    # Days_Since_First -0.0004549675 0.0002601691
```
...
ggplot()+
  geom_point(data=prev_fishbay, aes(x = Date_nice, y=Prev_Percent), na.rm = TRUE, color="firebrick2", size = 4) +
  geom_smooth(data=prev_fishbay, aes(x = Date_nice, y=Prev_Percent), na.rm = TRUE, method = loess, color="firebrick2", se = FALSE, size=2) +
  geom_point(data=prev_coralbay, aes(x = Date_nice, y=Prev_Percent), na.rm = TRUE, color="firebrick4", size = 4) +
  geom_smooth(data=prev_coralbay, aes(x = Date_nice, y=Prev_Percent), na.rm = TRUE, method = loess, color="firebrick4", se = FALSE, size=2) +
  scale_y_continuous(name = "SCTLD prevalence (percent)") +
  xlab("") +
  theme_classic() +
  theme(text=element_text(size=16))
```
...
# 
#       ASV Mean_Abundance SD_Abundance
# ASV1 ASV1     0.16417703   0.07242084
# ASV5 ASV5     0.05082851   0.02772590

```

```{r}
#healthy and recovered corals at Fish Bay
rm(list = setdiff(ls(), c("ps.decontam.filt", "ps.seawater.Fishbay", "ps.Seawater.Coralbay", "taxa")))

#pull out only healthy and recovered corals
ps.SW.FB.HorR <- ps.seawater.Fishbay %>%
  subset_samples(Healthy_Diseased_Recovered != "Dead") %>% 
  subset_samples(Healthy_Diseased_Recovered != "Diseased")  

asv_ids <- c("ASV1", "ASV5")

# Extract the abundance data from the phyloseq object
otu_data <- otu_table(ps.SW.FB.HorR)
# Ensure OTU table is in a matrix format (required for some phyloseq objects)
if (!taxa_are_rows(ps.SW.FB.HorR)) {
  otu_data <- t(otu_data)
}
# Subset the OTU table for the selected ASVs
selected_asv_data <- as.data.frame(t(otu_data[asv_ids, , drop = FALSE]))
# sum the rel abun of the two taxa
selected_asv_data$ASV_sum <- selected_asv_data$ASV1 + selected_asv_data$ASV5

mean_1and5 <- mean(selected_asv_data$ASV_sum) #0.246066
SD_1and5 <- sd(selected_asv_data$ASV_sum) #0.07664803
# this is the mean and SD of the two most abundant pro and syn in fish bay SW amongst healthy and recovered corals. 
```

Now I want to determine which dates there are less than Mean-2*SD of ASV1 and ASV 5. 
```{r}
threshold <- mean_1and5 - 2*SD_1and5 #0.09276994

rm(list = setdiff(ls(), c("ps.decontam.filt", "ps.seawater.Fishbay", "taxa", "threshold", "asv_ids")))

#looking at all the SW FB data, not just the healthy and recovered

# Extract the abundance data from the phyloseq object with all samples
otu_data <- otu_table(ps.seawater.Fishbay)
# Ensure OTU table is in a matrix format (required for some phyloseq objects)
if (!taxa_are_rows(ps.seawater.Fishbay)) {
  otu_data <- t(otu_data)
...
selected_asv_data$ASV_sum <- selected_asv_data$ASV1 + selected_asv_data$ASV5
# Extract metadata
metadata <- as.data.frame(sample_data(ps.seawater.Fishbay))
# Merge metadata with the ASV abundance data
selected_asv_data <- cbind(metadata, selected_asv_data)

filtered_data <- selected_asv_data %>% filter(ASV_sum < threshold)
```


```{r}
rm(list = setdiff(ls(), c("ps.decontam.filt", "ps.seawater.Fishbay", "taxa", "threshold", "filtered_data")))

samples_LowCyano <-filtered_data$Sample_ID
  # "Seawater_29" "Seawater_32" "Seawater_34_duplicate" "Seawater_41" 
ps.LowCyano <- ps.seawater.Fishbay %>%
  subset_samples(Sample_ID %in% c("Seawater_29", "Seawater_32", "Seawater_34_duplicate", "Seawater_41"))

# Extract the abundance data from the phyloseq object with all samples
otu_data <- otu_table(ps.LowCyano)
# Ensure OTU table is in a matrix format (required for some phyloseq objects)
if (!taxa_are_rows(ps.LowCyano)) {
  otu_data <- t(otu_data)
}
otu_df <- as.data.frame(otu_data)

otu_df$sum <- rowSums(otu_df, na.rm = TRUE)
#otu_df$mean <- rowMeans(otu_df[,1:4], na.rm = TRUE)

otu_df <- otu_df[order(-otu_df$sum), ]
...
rm(list = setdiff(ls(), c("ps.decontam.filt", "ps.seawater.Fishbay", "ps.Seawater.Coralbay", "asv_abundance")))
...
ASVofInterst <- c("ASV1", "ASV5", "ASV45", "ASV67", "ASV79", "ASV84")
...
colNeeded <- c("ASV1", "ASV5", "ASV45", "ASV67", "ASV79", "ASV84", "Sample_names", "Site", "Tag", "Date", "Sample_Type", "Healthy_Diseased_Recovered")
...
ggplot(combo_data_forplotting, aes(x = Date, y = RelativeAbundance, color = ASV, group = ASV)) +
  ...
  theme(
    ...
    scale_color_manual(breaks=c("ASV1", "ASV5", "ASV45", "ASV67", "ASV79", "ASV84"), values = c("#009E73", "#996FD2", "#CC79A7", "#D55E00", "#0072B2", "#E69F00"))
...
colNeeded <- c("ASV1", "ASV5", "ASV45", "ASV67", "ASV79", "ASV84", "Sample_names", "Site", "Tag", "Date", "Sample_Type", "Healthy_Diseased_Recovered", "Site_Disease")
combo_data <- combo_data[,colNeeded]
```
